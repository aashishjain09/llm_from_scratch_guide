...
class MLP(nn.Module):
    ...
    It expands the input embedding size, applies the ReLU activation, and then projects it back
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Args:
            x (torch.Tensor): Input tensor of shape (B, T, C), where B is batch size, T is sequence length, and C is embedding size.
    ...
    def forward_embedding(self, x: Tensor) -> Tensor:
        """
        Applies the hidden linear layer followrd by ReLU activation.
